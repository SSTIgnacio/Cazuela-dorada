...
const flowWelcome = addKeyword(EVENTS.WELCOME)
    .addAnswer(
        "Te damos la bienvenida *Turismo Guácimo*!", // Mensaje de bienvenida
...
const flowOpciones = addKeyword(EVENTS.ACTION)
    .addAnswer(opciones, // Mensaje con las opciones disponibles
        ...
        async (ctx, { gotoFlow, fallBack, flowDynamic }) => {
            ...
            switch (ctx.body) {
                case "1":
                    return gotoFlow(flowLugTur); // Redirige al flujo de lugares turisticos
                case "2":
...
// Flujo para mostrar el menú del restaurante en formato PNG
const flowLugTur = addKeyword(EVENTS.ACTION)
    .addAnswer('Aquí tienes nuestro menú', {
        media: pathMenuPNG // Enviar el archivo PNG del menú
    })
...
const flowReservaMesa = addKeyword('ACTION')
    .addAnswer(
        ...
        async (ctx, { flowDynamic, fallBack }) => {
            ...
            try {
                // Se define la ruta al archivo "reservadisponible.txt" que contiene las reservas existentes
                const pathdisponible = path.join(__dirname, "mensajes", "reservadisponible.txt");
            
                // Este objeto representa la disponibilidad inicial de mesas por horario
                let mesasDisponibles = { ...mesasDisponiblesPorHorario };
            
                // Si el archivo de disponibilidad existe, se procede a leer su contenido
                if (fs.existsSync(pathdisponible)) {
                    // Se lee el contenido del archivo de reservas
                    const fileContent = fs.readFileSync(pathdisponible, "utf8");
            
                    // Se filtran las reservas que corresponden al día de la reserva (diaReserva)
                    const reservas = fileContent.split('\n').filter(line => line.includes(diaReserva));
            
                    // Se cuenta cuántas mesas están ocupadas en cada horario utilizando un reduce
                    const conteoMesasPorHorario = reservas.reduce((acc, reserva) => {
                        // Para encontrar los horarios en formato "hh:mm a hh:mm"
                        const horarioOcupado = reserva.match(/(\d{1,2}:\d{2}) a (\d{1,2}:\d{2})/);
                        if (horarioOcupado) {
                            // Se construye un rango de horario
                            const rangoOcupado = `${horarioOcupado[1]} - ${horarioOcupado[2]}`;
                            
                            // Se cuenta cuántas veces aparece ese rango de horario (mesas ocupadas)
                            if (acc[rangoOcupado]) {
                                acc[rangoOcupado]++;
                            } else {
                                acc[rangoOcupado] = 1;
                    ...
                    // Se actualiza la disponibilidad de mesas restando las mesas ocupadas
                    for (const [horario, ocupadas] of Object.entries(conteoMesasPorHorario)) {
                        // Si el horario existe en `mesasDisponibles`, se resta el número de mesas ocupadas
                ...
                // Si el día de la reserva es el día de hoy, se filtran los horarios que ya han pasado
                if (parseInt(dia) === fechaHoy.getDate()) {
                    // Se obtiene la hora actual
                    const horaActual = ahora.getHours();
            
                    // Se filtran los horarios, manteniendo solo los que son posteriores a la hora actual
                ...
                // Se eliminan los horarios donde ya no hay mesas disponibles (0 mesas)
                mesasDisponibles = Object.fromEntries(
    ...
    .addAnswer("Responde con *Sí* o *No* para continuar.", { capture: true }, async (ctx, { flowDynamic, fallBack }) => {
        try {
            ...
            if (response === 'si') {
                ...
                await flowDynamic("Si necesitas más ayuda, no dudes en contactarnos."); // Mensaje de agradecimiento
            } else {
...
const main = async () => {
    try {
        ...
        const adapterFlow = createFlow([flowWelcome, flowReservaMesa, flowPago, flowOpciones, flowMenu, flowLugTur, flowConsultas]);
